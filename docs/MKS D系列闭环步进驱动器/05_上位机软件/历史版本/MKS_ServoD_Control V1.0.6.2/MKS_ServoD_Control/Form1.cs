...
namespace MKS_ServoD_Control
{
    public partial class Form1 : Form
    {
        ...
        string[] HardVer_Error = { "检测到当前电机为ServoE系列，请使用MKS ServoE Control上位机。", "Detected that the current motor is ServoE series, please use the MKS ServoE Control." };
        string Error_HardVer;
            ...
            if (Addr <= 0xff && Addr >= 0x00)
            {
                ...
                if (DataSended == 31) //(31 限位参数)
                {
                    try
                    {
                        ...
                        if (SpeedText <= 3000 && SpeedText >= 1)
                        {
                            if (GB5_comboBox_触发电平.Text.Length != 0 && GB5_comboBox_归零方向.Text.Length != 0 && GB5_comboBox_归零使能.Text.Length != 0)
                            {
                ...
                if (DataSended == 35) //(35 无限位开关回零)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (GB5_comboBox_无限位回零模式.Text.Length != 0)
                            {
                ...
                if (DataSended == 47) //(47 保持电流-确认)
                {
                    ...
                    if (GB3_comboBox_保持电流.Text.Length != 0)
                    {
                ...
                if (DataSended == 49) //(49 单圈回零)
                {
                    try
                    {
                        ...
                        if (GB5_comboBox_回零模式.Text.Length != 0 && GB5_comboBox_设置零点.Text.Length != 0 && GB5_comboBox_回零方向.Text.Length != 0)
                        {
                ...
                if (DataSended == 56) //(56 EN回零与位置保护)
                {
                    try
                    {
                        ...
                        if (GB5_comboBox_EN回零.Text.Length != 0 && GB5_comboBox_位置保护.Text.Length != 0)
                        {
                ...
                if (DataSended == 57) //(57 写IO端口)
                {
                    try
                    {
                        ...
                        if (GB5_comboBox_OUT2写入.Text.Length != 0 && GB5_comboBox_OUT1写入.Text.Length != 0)
                        {
            ...
            if ((byte)rCHK == SerialBuffer_RX[Data_Number - 1])
            {
                switch ( SerialBuffer_RX[2] )
                {
                    ...
                    case 0x40:// 43 版本信息
                        try
                        {
                            if ((SerialBuffer_RX[3] & 0x80) == 0x80)
                            {
                            ...
                            if ((SerialBuffer_RX[3] & 0x0f) <= 8 && (SerialBuffer_RX[3] & 0x0f) >= 1)
                            {
                                HV = HardVersion[SerialBuffer_RX[3] & 0x0f];
                            }
                            else
                            {
                                HV = HardVersion[0];
                            ...
                            if (DataSended == 43)
                            {
                                ...
                                {
                                    MessageBox.Show(Error_HardVer, Text2_window);
                                }
                                MessageBox.Show(Text_HardVer + ":  " + HV + "\n" + Text_FirmVer + ":  " + FV, "Version");
                            }
                            if (HV == "MKS SERVO57D_485" || HV == "MKS SERVO57D_CAN" || HV == "MKS SERVO57E_485" || HV == "MKS SERVO57E_CAN")
                            {
                                Max_Current = 5200;
        ...
        private void Form1_Load_1(object sender, EventArgs e) //启动程序-初始化设置
        {
            ...
            F1_label_提示.Size = new Size(274, 30);
            F1_label_提示.Text = "提示：鼠标悬停在按键上可获取功能码（Code），可在说明书内获取更详细的说明。";
        ...
        private void englishToolStripMenuItem_Click(object sender, EventArgs e) //英文文本
        {
            ...
            F1_label_提示.Size = new Size(410, 30);
            F1_label_提示.Text = "Tip: Hovering the mouse over the button can obtain the function code, and more detailed instructions can be obtained in the manual.";
